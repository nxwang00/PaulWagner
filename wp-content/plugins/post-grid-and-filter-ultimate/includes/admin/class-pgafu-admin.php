...
class PGAFU_Admin {
	...
	function pgafu_plugin_row_meta( $links, $file ) {
		...
		if ( $file == PGAFU_PLUGIN_BASENAME ) {
			...
			$row_meta = array(
				'docs'    => '<a href="' . esc_url('https://docs.essentialplugin.com/post-grid-and-filter-ultimate/?utm_source=post_grid_filter&utm_medium=plugin_list&utm_campaign=plugin_quick_link') . '" title="' . esc_attr( __( 'View Documentation', 'post-grid-and-filter-ultimate' ) ) . '" target="_blank">' . __( 'Docs', 'post-grid-and-filter-ultimate' ) . '</a>',
