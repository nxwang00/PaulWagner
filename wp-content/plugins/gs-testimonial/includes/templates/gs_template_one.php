...
if ( $gs_t_loop->have_posts() ) {
	$output .= $this->gs_cycle_switcher( $theme, $transition, $gs_t_slider_stop, $gs_t_responsive, $speed,$gs_t_slider_column );
	...
	while ( $gs_t_loop->have_posts() ) {
		...
		$gs_testimonial_url = wp_get_attachment_image_src( $gs_testimonial_id, [ 86,86 ],true );
		...
		$output .= '<div class="gs_testimonial_single">';
			$output .= '<div class="testimonial-box">';
				if ( ! empty( $gs_testimonial_id ) && 'on' === $gs_t_image ) {
	...
	$output .= __( 'No Testimonial Added!', 'gst' );
}
